...
@section('content')
    <div class="row">
        ...
        <div class="col-12 mb-4">
            <a href="{{ route('clearing-house-enrollments.index') }}" class="btn btn-outline-secondary">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered mt-3">
                        <tbody>
                            ...
                            <tr>
                                <th>Company Phone:</th>
                                <td>{{ $enrollment->company_phone ?: '—' }}</td>
                            </tr>
                            ...
                            <tr class="table-info">
                                <th colspan="2" class="font-weight-bold">FMCSA Clearinghouse Onboarding</th>
                            </tr>
                            <tr>
                                <th>Owner-Operator:</th>
                                <td>{{ $enrollment->is_owner_operator ? 'Yes' : 'No' }}</td>
                            </tr>
                            <tr>
                                <th>Clearinghouse Registered:</th>
                                <td>{{ ucfirst($enrollment->clearinghouse_registered) }}</td>
                            </tr>
                            <tr>
                                <th>Authorize Conduct Queries:</th>
                                <td>{{ $enrollment->authorize_conduct_queries ? 'Yes' : 'No' }}</td>
                            </tr>
                            <tr>
                                <th>Authorize Report Violations:</th>
                                <td>{{ $enrollment->authorize_report_violations ? 'Yes' : 'No' }}</td>
                            </tr>
                            <tr>
                                <th>Authorize Report RTD:</th>
                                <td>{{ $enrollment->authorize_report_rtd ? 'Yes' : 'No' }}</td>
                            </tr>
                            <tr>
                                <th>Acknowledged Designate C/TPA:</th>
                                <td>{{ $enrollment->acknowledge_designate_ctpa ? 'Yes' : 'No' }}</td>
                            </tr>
                            <tr>
                                <th>Acknowledged Query Plan:</th>
                                <td>{{ $enrollment->acknowledge_query_plan ? 'Yes' : 'No' }}</td>
                            </tr>
        ...
        <div class="col-lg-5 box-margin">
            ...
            <div class="card card-body mb-4">
                ...
                <div class="my-3 text-center">
                    <span class="{{ $enrollment->status_badge_class }} font-weight-bold font-16 px-4 py-2 d-inline-block"
                ...
                @else
                    <form action="{{ route('clearing-house-enrollments.updateStatus', $enrollment->id) }}" method="POST" class="mb-4">
                        @method('PUT')
                        @csrf
                @endif
                    <div class="form-group mb-0">
                        <label for="status" class="font-weight-bold">Update Compliance Status:</label>
                ...
                <div class="alert alert-light border p-3 mb-0 mt-3">
                    <h6 class="font-weight-bold mb-2"><i class="fa fa-cc-stripe mr-2 text-primary"></i>Stripe Details</h6>
                    <div class="small">
                        <div><strong>Checkout Session:</strong><br><code class="d-inline-block text-break">{{ $enrollment->stripe_checkout_session_id ?: '—' }}</code></div>
                        <div class="mt-2"><strong>Payment Intent ID:</strong><br><code class="d-inline-block text-break">{{ $enrollment->stripe_payment_intent_id ?: '—' }}</code></div>
                    </div>
                </div>
            </div>
            ...
            <div class="card card-body">
                ...
                @if ($demo_mode == 'on')
                    @include('admin.demo_mode.demo-mode')
                @else
                    <form action="{{ route('clearing-house-enrollments.updateNotes', $enrollment->id) }}" method="POST" class="mb-4">
                        @method('PUT')
                        @csrf
                @endif
