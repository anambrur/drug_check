            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-20">
                        <h6 class="card-title mb-0">{{ __('content.packages') }}
                            <!-- Button -->
                        ...
                        <div>
                            ...
                            <a href="{{ url('admin/package/create') }}" class="btn btn-primary float-right mb-3">+
                                {{ __('content.add_package') }}</a>
                        </div>
                    </div>
                    ...
                    @if (count($packages) > 0)
                        ...
                        @else
                            <form onsubmit="return btnCheckListGet()" action="{{ route('package.destroy_checked') }}"
                                method="POST">
                                @method('DELETE')
                                @csrf
                        @endif
                        ...
                        <table id="basic-datatable" class="table table-striped dt-responsive w-100">
                            ...
                            <tbody>
                                ...
                                @foreach ($packages as $package)
                                    <tr>
                                        <td>
                                            <input name="check_list[]" type="checkbox" value="{{ $package->id }}"
                                        ...
                                        <td><span class="badge badge-pill badge-dark">
                                                {{ $package->package_category->category_name }}
                                            </span></td>
                                        <td>{{ $package->result }}</td>
                                        <td>{{ $package->price }}</td>
                                        <td>{{ $package->order }}</td>
                                        <td>
                                            @if ($package->status == 'published')
                                                <span
                                                    class="badge badge-pill badge-success">{{ __('content.published') }}</span>
                                            @else
                                                <span
                                                    class="badge badge-pill badge-danger">{{ __('content.draft') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div>
                                                <a href="{{ route('package.edit', $package->id) }}" class="mr-2">
                                    ...
                                    <div class="modal fade" id="deleteModal{{ $package->id }}" tabindex="-1"
                                        role="dialog" aria-labelledby="packageModalCenterTitle" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="packageModalCenterTitle">
                                                        {{ __('content.delete') }}</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="{{ __('content.close') }}">
                                                        <span aria-hidden="true">&times;</span>
