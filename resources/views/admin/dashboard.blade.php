...
@section('content')
    <style>
        ...
        .dash-welcome:hover .welcome-icon-box {
        ...
        .stat-grid .stat-card-wrapper:nth-child(2) {
            animation-delay: .1s
        }

        .stat-grid .stat-card-wrapper:nth-child(3) {
            animation-delay: .15s
        }

        .stat-grid .stat-card-wrapper:nth-child(4) {
            animation-delay: .2s
        }

        .stat-grid .stat-card-wrapper:nth-child(5) {
            animation-delay: .25s
        }

        .stat-grid .stat-card-wrapper:nth-child(6) {
            animation-delay: .3s
        }
        ...
        @if (isset($user_type) && $user_type === 'super-admin')
            ...
            @if (isset($monthly_trends) && $monthly_trends->isNotEmpty())
                <div class="row">
                    <div class="col-12 mb-4">
                        <div class="card dash-card">
                            <div class="card-header">
                                <h5><i class="fas fa-chart-line mr-2" style="color:#10b981"></i> Monthly Trends —
                                    {{ now()->year }}</h5>
            ...
            <div class="row">
                <div class="col-lg-8 mb-4">
                    <div class="card dash-card">
                        <div class="card-header">
                            ...
                            <a href="{{ url('admin/quest-order') }}" class="badge badge-primary"
                                    ...
                                    <tbody>
                                        @forelse($recent_activities as $order)
                                            <tr>
                                                ...
                                                <td><code
                                                        style="font-size:.8rem">{{ $order['reference_id'] ?? '-' }}</code>
                                                </td>
                                                <td>
                                                    @php $s = strtolower($order['status'] ?? ''); @endphp
                                                    <span
                                                        class="status-badge badge-{{ str_contains($s, 'complet') ? 'completed' : (str_contains($s, 'pend') ? 'pending' : 'cancelled') }}">{{ $order['status'] ?? 'N/A' }}</span>
                                                </td>
                                                <td>
                                                    @if ($order['result'])
                                                        @php $r = strtolower($order['result']); @endphp
                                                        <span
                                                            class="status-badge badge-{{ str_contains($r, 'neg') ? 'negative' : (str_contains($r, 'pos') ? 'positive' : 'pending') }}">{{ $order['result'] }}</span>
                                                    @else
                ...
                <div class="col-lg-4 mb-4">
                        ...
                        <div class="card-body">
                            ...
                            @forelse($top_clients as $i => $client)
                                <div class="top-client-item">
                                    <div class="client-avatar" style="background:{{ $avatarColors[$i % 5] }}">
                                        {{ strtoupper(substr($client->company_name ?? '?', 0, 2)) }}</div>
                    ...
                    <div class="card dash-card">
                        <div class="card-header">
                            <h5><i class="fas fa-bolt mr-2" style="color:#6366f1"></i> Quick Actions</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <a href="{{ url('admin/client-profile/create') }}" class="quick-action">
                                <i class="fas fa-user-plus" style="background:#d1fae5;color:#10b981"></i><span>Add
                                    Client</span>
                            </a>
                            <a href="{{ url('admin/result-recording') }}" class="quick-action">
                                <i class="fas fa-clipboard-list" style="background:#fef3c7;color:#f59e0b"></i><span>View
                                    Results</span>
                            </a>
                            <a href="{{ url('admin/report/mis-reports') }}" class="quick-action">
                                <i class="fas fa-chart-bar" style="background:#ede9fe;color:#8b5cf6"></i><span>MIS
                                    Reports</span>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="col-12 mb-4">
                        <div class="card dash-card">
                            <div class="card-header">
                                ...
                                <a href="{{ url('admin/result-recording') }}" class="badge badge-success"
                                    style="text-decoration:none">View All →</a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                        ...
                                        <tbody>
                                            @foreach ($recent_results as $result)
                                                <tr>
                                                    <td>
                                                        <span
                                                            class="emp-avatar">{{ strtoupper(substr(optional($result->employee)->first_name ?? '?', 0, 1)) }}</span>
                                                        {{ optional($result->employee)->first_name }}
                                                        {{ optional($result->employee)->last_name }}
                                                    </td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <span
                                                            class="status-badge badge-{{ str_contains($rs, 'complet') ? 'completed' : 'pending' }}">{{ ucfirst($result->status ?? 'N/A') }}</span>
                                                    </td>
                                                    <td><small>{{ $result->reason_for_test ?? '-' }}</small></td>
                                                    <td><small
                                                            class="text-muted">{{ \Carbon\Carbon::parse($result->created_at)->format('m/d/Y') }}</small>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                    ...
                    <div class="col-12 mb-4">
                        <div class="card dash-card">
                            <div class="card-header">
                                ...
                                <a href="{{ url('admin/payments') }}" class="badge badge-primary"
                                    style="text-decoration:none">View All →</a>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-modern mb-0">
                                        ...
                                        <tbody>
                                            @foreach ($recent_payments as $payment)
                                                <tr>
                                                    <td><code
                                                            style="font-size:.8rem">{{ $payment->payment_id ?: '-' }}</code>
                                                    </td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <span
                                                            class="status-badge badge-{{ $ps === 'succeeded' ? 'completed' : ($ps === 'refunded' ? 'negative' : 'pending') }}">{{ ucfirst($payment->status ?? 'N/A') }}</span>
                                                    </td>
                                                    <td><small
                                                            class="text-muted">{{ \Carbon\Carbon::parse($payment->created_at)->format('m/d/Y h:i A') }}</small>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
            ...
            @else
                ...
                @if (isset($company_profile))
                    <div class="row mb-3">
                        <div class="col-12">
                            <div
                                style="background:linear-gradient(135deg,#0ea5e9,#6366f1);border-radius:14px;padding:18px 24px;color:#fff;display:flex;align-items:center;gap:18px">
                                <div
                                    style="width:50px;height:50px;border-radius:12px;background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;font-size:1.3rem;font-weight:700">
                                    {{ strtoupper(substr($company_profile->company_name ?? '?', 0, 2)) }}
                                </div>
                                <div>
                                    <h5 style="margin:0;font-weight:700">{{ $company_profile->company_name }}</h5>
                                    <small style="opacity:.8">Account #{{ $company_profile->account_no ?? 'N/A' }} ·
                                        {{ $company_profile->city ?? '' }}, {{ $company_profile->state ?? '' }}</small>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-8 mb-4">
                        <div class="card dash-card">
                            <div class="card-header">
                                <h5><i class="fas fa-chart-bar mr-2" style="color:#6366f1"></i> Weekly Orders</h5><span
                                    class="badge badge-primary">Last 7 Days</span>
                            </div>
                    ...
                    <div class="col-lg-4 mb-4">
                        <div class="card dash-card">
                            ...
                            <div class="card-body d-flex align-items-center justify-content-center"><canvas
                                    id="orderStatusChart" style="max-height:240px"></canvas></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-lg-6 mb-4">
                        <div class="card dash-card">
                            <div class="card-header">
                                <h5><i class="fas fa-users mr-2" style="color:#10b981"></i> Employees</h5>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-modern mb-0">
                                        ...
                                        <tbody>
                                            @forelse($company_employees as $emp)
                                                <tr>
                                                    <td><span
                                                            class="emp-avatar">{{ strtoupper(substr($emp->first_name ?? '?', 0, 1)) }}</span>{{ $emp->first_name }}
                                                        {{ $emp->last_name }}</td>
                                                    <td>{{ $emp->department ?? '-' }}</td>
                                                    <td><span
                                                            class="status-badge badge-{{ strtolower($emp->status ?? '') === 'active' ? 'completed' : 'pending' }}">{{ ucfirst($emp->status ?? 'N/A') }}</span>
                                                    </td>
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="3" class="text-center text-muted py-4">No employees
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                            ...
                                            @forelse($recent_activities as $order)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php $s = strtolower($order['status'] ?? ''); @endphp
                                                        <span
                                                            class="status-badge badge-{{ str_contains($s, 'complet') ? 'completed' : 'pending' }}">{{ $order['status'] ?? 'N/A' }}</span>
                                                    </td>
                                                    <td>
                                                        @php $r = strtolower($order['result'] ?? ''); @endphp
                                                        <span
                                                            class="status-badge badge-{{ str_contains($r, 'positive') ? 'danger' : 'success' }}">{{ $order['result'] ?? 'N/A' }}</span>
                                                    </td>
                                                    <td><small class="text-muted">{{ $order['time_ago'] }}</small></td>
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="4" class="text-center text-muted py-4">No recent
                                                        orders</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-modern mb-0">
                                            ...
                                            <tbody>
                                                @foreach ($recent_results as $result)
                                                    <tr>
                                                        <td>{{ optional($result->employee)->first_name }}
                                                            {{ optional($result->employee)->last_name }}</td>
                                                        <td>{{ optional($result->laboratory)->laboratory_name ?? '-' }}
                                                        </td>
                                                        <td>{{ optional($result->mro)->name ?? '-' }}</td>
                                                        <td><span
                                                                class="status-badge badge-{{ strtolower($result->status ?? '') === 'completed' ? 'completed' : 'pending' }}">{{ ucfirst($result->status ?? 'N/A') }}</span>
                                                        </td>
                                                        <td><small>{{ \Carbon\Carbon::parse($result->created_at)->format('m/d/Y') }}</small>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const palette = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#0ea5e9', '#f43f5e', '#14b8a6',
                    '#e11d48', '#84cc16'
                ];
                    ...
                    new Chart(document.getElementById('weeklyOrdersChart'), {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                backgroundColor: palette.slice(0, Object.keys(weeklyData).length).map(
                                    c => c + '33'),
                ...
                @if (isset($monthly_trends) && $monthly_trends->isNotEmpty())
                    ...
                    new Chart(document.getElementById('monthlyTrendChart'), {
                        ...
                        data: {
                            ...
                            datasets: [{
                                    label: 'Orders',
                                    data: mtLabels.map(k => mt[k].orders),
                                    borderColor: '#6366f1',
                                ...
                                {
                                    label: 'Completed',
                                    data: mtLabels.map(k => mt[k].completed),
                                    borderColor: '#10b981',
