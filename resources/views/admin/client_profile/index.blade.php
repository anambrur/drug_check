            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-20">
                        ...
                        <div>
                            <a href="{{ url('admin/client-profile/create') }}" class="btn btn-primary float-right mb-3">+ Add Client Profile</a>
                        </div>
                    </div>
                    ...
                    @if (count($clientProfiles) > 0)
                        ...
                        @else
                            <form onsubmit="return btnCheckListGet()" action="{{ route('client-profile.destroy_checked') }}" method="POST">
                                @method('DELETE')
                                @csrf
                                @endif
                            ...
                            <table id="basic-datatable" class="table table-striped dt-responsive w-100">
                                ...
                                @foreach ($clientProfiles as $clientProfile)
                                    <tr>
                                        ...
                                        <td>
                                            <div>
                                                <a href="{{ route('client-profile.edit', $clientProfile->id) }}" class="mr-2">
                                                    <i class="fa fa-edit text-info font-18"></i>
                                                </a>
                                                <a href="{{ route('client-profile.show', $clientProfile->id) }}" class="mr-2">
                                                    <i class="fa fa-eye text-primary font-18"></i>
                                                </a>
                                                <a href="#" data-toggle="modal" data-target="#deleteModal{{ $clientProfile->id }}">
                                                    <i class="fa fa-trash text-danger font-18"></i>
                                                </a>
                                            </div>
                                        </td>
                                    ...
                                    <!-- Modal -->
                                    <div class="modal fade" id="deleteModal{{ $clientProfile->id }}" tabindex="-1" role="dialog" aria-labelledby="clientProfileModalCenterTitle" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="clientProfileModalCenterTitle">{{ __('content.delete') }}</h5>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('content.close') }}">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                ...
                                                <div class="modal-footer">
                                                    @if ($demo_mode == "on")
                                                        ...
                                                        @include('admin.demo_mode.demo-mode')
                                                    @else
                                                        <form class="d-inline-block" action="{{ route('client-profile.destroy', $clientProfile->id) }}" method="POST">
                                                            @method('DELETE')
                                                            @csrf
                                                            @endif
