...
@section('content')
    ...
    @php
        ...
        $heroTitle = $searchQuery !== ''
            ? __('frontend.search') . ': “' . $searchQuery . '”'
            : ($page_builder->breadcrumb_title ?? $page_builder->meta_title ?? __('frontend.search'));
    @endphp

    <div class="svc-page ch-page" id="blog">
        <div class="rc-scroll-progress" id="rc-scroll-progress" aria-hidden="true"><span></span></div>

        <section class="rc-hero rc-hero--blog rc-hero--search">
            ...
            <div class="container position-relative">
                <div class="rc-hero-content text-center">
                    <span class="rc-badge rc-hero-item rc-hero-item--1">
                        ...
                        {{ $totalPosts }} {{ \Illuminate\Support\Str::plural('Result', $totalPosts) }}
                    </span>
                    <h1 class="rc-hero-title rc-hero-item rc-hero-item--2" id="bl-search-heading">{{ $heroTitle }}</h1>

                    <form action="{{ route('default-blog-search-index') }}" method="POST" class="rc-hero-search rc-hero-item rc-hero-item--3">
                        @csrf
                        <div class="bl-search bl-search--hero">
                            <input type="search"
                                   name="search"
                                   value="{{ $searchQuery }}"
                                   ...
                                   required>
                            <button type="submit" class="bl-search-btn" aria-label="{{ __('frontend.search') }}">
                                <i class="fas fa-search" aria-hidden="true"></i>
                            </button>
                        </div>
                    </form>
                    ...
                    @if (is_countable($blog_count_categories) && count($blog_count_categories) > 0)
                        <nav class="rc-cat-bar rc-hero-item rc-hero-item--4" aria-label="Blog categories">
                ...
                @else
                    <div class="pf-card svc-empty-card text-center rc-animate">
                        <div class="pf-body py-5">
                            ...
                            <p class="ch-text-muted mb-3">
                                @if ($searchQuery !== '')
                                    No articles matched “{{ $searchQuery }}”.
                                @else
                                    Try a different keyword to find related articles.
                                @endif
                            </p>
                            @if (!empty($blog_index?->page_uri))
