                ...
                @isset($service_content)
                    <div class="services-detail-top">
                        @if (!empty($service_content->section_image))
                            <img src="{{ asset('uploads/img/service/' . $service_content->section_image) }}"
                                alt="Services image" class="img-fluid">
                            @if ($service->type == 'image')
                                @if (!empty($service->section_image))
                                    <span>
                                        <img src="{{ asset('uploads/img/service/' . $service->section_image) }}"
                                            alt="Services image" class="img-fluid">
                                    </span>
                                @endif
                            @else
                    ...
                    <div class="services-detail-inner custom-blog-img">
                    ...
                    <div class="col-md-6">
                        <div class="design-process-video">
                            @if (!empty($service_info->section_image))
                                <img src="{{ asset('uploads/img/service/' . $service_info->section_image) }}"
                                    alt="service info" class="img-fluid">
                            @endif
                            @if (!empty($service_info->video_url))
                                @if ($service_info->video_type == 'youtube')
                                    <a class="design-process-video-btn" href="{{ $service_info->video_url }}"><i
                                            class="fa fa-play"></i></a>
                                @else
                                    <a class="design-process-video-btn-2" href="{{ $service_info->video_url }}"><i
                                            class="fa fa-play"></i></a>
                                @endif
                            @endif
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="design-process-video">
                                ...
                                <a class="design-process-video-btn" href="https://www.youtube.com/watch?v=YqQx75OPRa0"><i
                                        class="fa fa-play"></i></a>
                            </div>
                        </div>
                        <div class="col-md-6">
                ...
                @else
                    @if (Auth::user() || $draft_view == null || $draft_view->status == 'enable')
                        <div class="sidebar-details-list">
                            <ul>
                                <li>
                                    <h6>Service Name<span>Web Design</span></h6>
                                </li>
                                <li>
                                    <h6>Service Industry<span>Web,App</span></h6>
                                </li>
                                <li>
                                    <h6>Service Duration<span>2 Weeks</span></h6>
                                </li>
                                <li>
                                    <h6>Service Total Hours<span>336 Hour</span></h6>
                                </li>
                            </ul>
                        </div>
                    @endif
            ...
            @foreach ($recent_services as $item)
                <div class="recent-post-item clearfix">
                    <div class="recent-post-img mr-3">
                            ...
                            <a href="{{ $item->button_url }}">
                                @if (!empty($item->section_image_2))
                                    <img src="{{ asset('uploads/img/service/' . $item->section_image_2) }}"
                                        class="img-fluid image-size-100" alt="service image">
                                @else
                        ...
                        @else
                            <a
                                href="{{ route('default-service-detail-show', ['service_slug' => $item->service_slug]) }}">
                                @if (!empty($item->section_image_2))
                                    <img src="{{ asset('uploads/img/service/' . $item->section_image_2) }}"
                                        class="img-fluid image-size-100" alt="service image">
                                @else
                    ...
                    <div class="recent-post-body">
                        ...
                        @endif
                        <p class="recent-post-date"><i
                                class="far fa-calendar-alt"></i>{{ Carbon\Carbon::parse($item->created_at)->isoFormat('DD') }}
                            {{ Carbon\Carbon::parse($item->created_at)->isoFormat('MMMM') }}
                            {{ Carbon\Carbon::parse($item->created_at)->isoFormat('GGGG') }}</p>
                    </div>
                </div>
            @endforeach
            @unset ($item)
