...
class PortfolioController extends Controller
{
    ...
    public function show($portfolio_slug = null)
    {
        ...
        return view('frontend.portfolio.show', compact(
            'preloader',
            'favicon',
            'seo',
            'google_analytic',
    ...
    public function category_index($category_name = null)
    {
        ...
        return view('frontend.portfolio.category-index', compact(
            'preloader',
            'favicon',
            'seo',
            'google_analytic',
            ...
            'menus',
            'portfolio_count_categories',
            'portfolio_section_paginate_style',
            'portfolios_paginate_style',
            'category',
            'footer_image_style1',
            'site_info',
            'footers',
    ...
    public function non_dot_testing()
    {
        $language = getSiteLanguage();
        ...
        $portfolios_style1 = Portfolio::join("portfolio_categories", 'portfolio_categories.id', '=', 'portfolios.category_id')
            ...
            ->where('portfolios.style', 'style1')
            ->where('portfolio_categories.status', 1)
            ->where('portfolio_categories.category_name', 'Non DOT Testing')
            ->where('portfolios.status', 'published')
        ...
        $portfolio_count_categories = Portfolio::select(DB::raw('count(*) as category_count, category_id'))
            ...
            ->groupBy('category_id')
            ->get();
        return view('frontend.portfolio.category_list.service', array_merge(
            getFrontendData(),
            compact('portfolios_style1', 'portfolio_count_categories')
        ));
    }

    public function dot_testing()
    {
        $language = getSiteLanguage();
        ...
        $portfolios_style1 = Portfolio::join("portfolio_categories", 'portfolio_categories.id', '=', 'portfolios.category_id')
            ...
            ->where('portfolios.style', 'style1')
            ->where('portfolio_categories.status', 1)
            ->where('portfolio_categories.category_name', 'DOT Testing')
            ->where('portfolios.status', 'published')
            ->orderBy('portfolios.order', 'asc')
            ->select('portfolios.*', 'portfolios.id')
            ->paginate(12);
