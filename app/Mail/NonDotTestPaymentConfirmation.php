...
class NonDotTestPaymentConfirmation extends Mailable implements ShouldQueue
{
    ...
    {
        $testName = $this->application->portfolio->title ?? 'Non-DOT Test';

        return new Envelope(
            subject: 'Non-DOT Payment Confirmation for ' . $testName,
            from: new Address(
    ...
    {
        return new Content(
            view: 'emails.non_dot_test_payment_confirmation',
            with: [
    ...
    {
        Log::error('Queued Non-DOT test payment confirmation email failed.', [
